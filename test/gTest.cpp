...
TEST(protocolTest, security01) {
	...
	bool result1 = decrypt(nullptr, header1, std::size(header1), message1, 0, 2, nonce, aesKey);
	bool result2 = decrypt(nullptr, header2, std::size(header2), message2, 0, 2, nonce, aesKey);
	EXPECT_TRUE(result1);
	EXPECT_TRUE(result2);
}
...
TEST(protocolTest, security10) {
	...
	Nonce nonce(deviceId, counter);
	bool result = decrypt(nullptr, header, std::size(header), message, 0, 4, nonce, aesKey);
	EXPECT_TRUE(result);
}
...
TEST(protocolTest, security11) {
	...
	uint8_t decrypted[1];
	bool result = decrypt(decrypted, header, std::size(header), message, 1, 4, nonce, aesKey);
	EXPECT_TRUE(result);
	EXPECT_EQ(decrypted[0], 0x20);

	uint8_t encrypted[5];
	encrypt(encrypted, header, std::size(header), decrypted, 1, 4, nonce, aesKey);
...
TEST(protocolTest, hash1) {
	...
	hash(output, input, std::size(input));
